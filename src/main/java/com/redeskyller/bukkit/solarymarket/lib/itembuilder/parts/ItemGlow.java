...
public class ItemGlow implements ItemPart {
	...
	public ItemStack send(ItemStack item)
	{
		if ((item != null) && (item.getType() != org.bukkit.Material.AIR) && (this.glow))
			try {
				Class<?> craftItemStackClass = obcClass("inventory.CraftItemStack");

				Class<?> nbtBaseClass = nmsClass("NBTBase");
				Class<?> nbtTagCompoundClass = nmsClass("NBTTagCompound");
				Class<?> nbtTagListClass = nmsClass("NBTTagList");
	...
	{
		try {
			return (Class.forName(nsmPackage.replace("<version>", nmsVersion()).concat(".").concat(className)));
		} catch (Exception exception) {
			exception.printStackTrace();
		}
		return null;
	...
	{
		try {
			return (Class.forName(obcPackage.replace("<version>", nmsVersion()).concat(".").concat(className)));
		} catch (Exception exception) {
			exception.printStackTrace();
		}
		return null;
