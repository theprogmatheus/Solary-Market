...
public class MapView
{
	...
	public MapView(ItemStack icone, Player player, String title) {
		...
		this.icone = icone;
		this.inventorys.put(Integer.valueOf(1), this.newPage(1));
		this.atual = 1;
	}
	...
	public void add(ItemStack item)
	{
		...
		if (inv.firstEmpty() >= 46) {
			this.inventorys.put(Integer.valueOf(this.inventorys.size() + 1), this.newPage(this.inventorys.size() + 1));
	...
	private ItemStack expiradosIcon()
	{
		...
		int size = this.expiradosSize(this.player);
		NBTItem itemnbt = new NBTItem(item);
		NBTTagCompound itemtag = itemnbt.getTag();
		NBTTagCompound newtag = new NBTTagCompound();
