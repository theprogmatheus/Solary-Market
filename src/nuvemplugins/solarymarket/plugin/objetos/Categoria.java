package nuvemplugins.solarymarket.plugin.objetos;
	...
	public void vizualizar(Player player)
	{
		...
		try {
			...
			ResultSet result = database
					.query("select * from " + SolaryMarket.table + "_mercado where categoria='" + this.getId() + "';");
			while (result.next()) {
				try {
							...
							try {
								...
								if (result.next()) {
									...
									if (event.getClick() == org.bukkit.event.inventory.ClickType.RIGHT) {
										this.vizualizarItens(player, itens, dono, preco,
										...
										this.confirmar(player, itens, dono, preco,
												UUID.fromString(hunterstag.getString("uuid")));
									} else {
